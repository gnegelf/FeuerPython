...
matlabData=scipy.io.loadmat('feuerData16_50.mat')
...
for i in range(contVarN+1,intVarN+contVarN+1):
    names[i-1]="bin"+str(i)
    if write:
        #print((names[i-1],int((i-contVarN)/tn),model.order.branch_direction.down))
        ...
        model.order.set([(names[i-1],int((i-contVarN)/tn),model.order.branch_direction.up)])
